...
IMU3DView::IMU3DView(QWidget *parent) : QOpenGLWidget(parent)
{
    mXRot = 0;
    mYRot = 0;
    mZRot = 0;
...
QSize IMU3DView::minimumSizeHint() const
{
    return QSize(0, 0);
}

QSize IMU3DView::sizeHint() const
{
    return QSize(0, 0);
}

void IMU3DView::setRollPitchYaw(double roll, double pitch, double yaw)
{
    mXRot = float(roll);
    mYRot = float(pitch);
    mZRot = float(yaw);
...
void IMU3DView::setQuanternions(float q0, float q1, float q2, float q3)
{
    mQ0 = q0;
    mQ1 = q1;
    mQ2 = q2;
...
void IMU3DView::makeObject()
{
    ...
    QImage imgSide(102, 512, QImage::Format_RGB32);
    imgSide.fill(Qt::darkGray);
    p = new QPainter(&imgSide);
    p->setPen(pen);
    p->drawRect(0, 0, 102, 512);
    delete p;

    mTextures[0] = new QOpenGLTexture(imgSq);
    mTextures[1] = new QOpenGLTexture(imgSide);
    mTextures[2] = new QOpenGLTexture(imgSide);
    mTextures[3] = new QOpenGLTexture(imgSide);
    mTextures[4] = new QOpenGLTexture(imgSide);
    mTextures[5] = new QOpenGLTexture(imgSq); //QOpenGLTexture(QImage(QString(":/res/images/v6plus_top.png")).mirrored());
