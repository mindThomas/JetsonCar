...
void LSPC_ConnectionThread(boost::shared_ptr<ros::NodeHandle> n, std::string serial_port, std::shared_ptr<std::timed_mutex> lspcMutex, std::shared_ptr<lspc::Socket *> lspcObj, std::future<void> exitSignalObj)
{
    ...
    ros::Publisher pub_encoders = n->advertise<jetsoncar_interfaces::Encoders>("encoders", 50);
...
int main(int argc, char **argv) {
    ...
    ros::Subscriber sub_setpoint = n->subscribe<jetsoncar_interfaces::Setpoint>("setpoint", 1, boost::bind(&ROS_Callback_Setpoint, _1, lspcMutex, lspcObj));
