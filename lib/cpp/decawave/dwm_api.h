...
#define ERR_BUSY		   (-2)
#define ERR_INVAL_ADDR	(-3)
#define ERR_INVAL_PARAM   (-4)
#define ERR_OVERRUN		(-5)
#define ERR_I2C_ANACK	(-10)
#define ERR_I2C_DNACK	(-11)
...
#define API_USR_DATA_LEN_MAX 		34
#define API_LABEL_LEN_MAX 			16
#define API_UWB_SCAN_RESULT_CNT_MAX 16
#define API_BH_ORIGIN_CNT_MAX 		9
/********************************************************************************************************************/
/*                                                     API LIST                                                     */
/********************************************************************************************************************/
 ...
 * @return Error code
 * @retval OK Success
 * @retval ERR_INTERNAL Position can't be read
 */
int pos_set_xyz(int32_t* p_x, int32_t* p_y, int32_t* p_z);//todo
 ...
 * @return Error code
 */
int pos_get(pos_t* pos);

/* maximum and minimum update rate in multiple of 100 ms */
enum upd_rate{
	UPD_RATE_MAX = 600,	/* 1 minute */
	UPD_RATE_MIN = 1	/* 100 ms */
};
/**
 * @brief Sets update rate
 *
 * @param[in] ur, Update rate in multiply of 100 ms, [min,max] = [UPD_RATE_MIN, UPD_RATE_MAX]
 * @param[in] urs, Stationary update rate in multiply of 100 ms, [min,max] = [UPD_RATE_MIN, UPD_RATE_MAX]
 *
 * @return Error code
 */
int upd_rate_set(uint16_t ur, uint16_t urs);

/**
 * @brief Gets update rate
 *
 * @param[out] ur, Pointer to update rate, update rate is multiply of 100 ms
 * [min,max] = [UPD_RATE_MIN, UPD_RATE_MAX]
 * @param[out] urs, Pointer to stationary update rate, update rate is multiply of 100 ms
 * [min,max] = [UPD_RATE_MIN, UPD_RATE_MAX]
 *
 * @return Error code
 */
int upd_rate_get(uint16_t *ur, uint16_t *urs);
 ...
 */
typedef enum {
	MEAS_MODE_TWR = 0,//!< MEAS_MODE_TWR
	MEAS_MODE_TDOA = 1//!< MEAS_MODE_TDOA
} meas_mode_t;
...
typedef enum {
	uwb_BH_ROUTING_OFF = 0,
	uwb_BH_ROUTING_ON = 1,
	uwb_BH_ROUTING_AUTO = 2,
} uwb_bh_routing_t;

typedef struct cfg_common {
	uwb_mode_t uwb_mode;
	bool fw_update_en;
	bool ble_en;
	bool led_en;
...
typedef struct cfg_anchor {
	cfg_common_t common;
	bool bridge;
	bool initiator;
	uwb_bh_routing_t uwb_bh_routing;
} cfg_anchor_t;

typedef struct cfg_tag {
	cfg_common_t common;
	bool loc_engine_en;
	bool low_power_en;
	bool stnry_en;
	meas_mode_t meas_mode;
} cfg_tag_t;
 ...
 * @return Error code
 */
int cfg_anchor_set(cfg_anchor_t* cfg);
...
#define RANGING_ANCHOR_CNT_MAX	14
...
typedef struct {
	uint8_t cnt;
	anchor_list_elt_t v[RANGING_ANCHOR_CNT_MAX];
} anchor_list_t;
 ...
 * @return Error code
 * @retval OK Success
 * @retval ERR_INVAL_ADDR null pointer
 * @retval ERR_INTERNAL internal error
 */
int anchor_list_get(anchor_list_t* p_list);
...
typedef struct {
	uint8_t cnt;
	uint16_t addr[RANGING_ANCHOR_CNT_MAX];
	uint32_t dist[RANGING_ANCHOR_CNT_MAX];
	uint8_t qf[RANGING_ANCHOR_CNT_MAX];
}distance_t;
...
typedef struct {
	uint8_t cnt;
	pos_t pos[RANGING_ANCHOR_CNT_MAX];
}anchor_pos_t;
 ...
 * @return Error code
 */
int baddr_get(baddr_t* baddr);

typedef enum {
	STNRY_SENSITIVITY_LOW = 0,
	STNRY_SENSITIVITY_NORMAL = 1,
	STNRY_SENSITIVITY_HIGH = 2
} stnry_sensitivity_t;

/**
 * @brief Writes configuration of the stationary mode which is used by tag node. The configuration
 * can be written even if stationary detection is disabled (see cfg_tag_set). Writes internal non-volatile memory
 ...
 * @return Error code
 * @retval OK Success
 * @retval ERR_INTERNAL failed to write configuration
 * @retval ERR_INVAL_PARAM unknown sensitivity level
 */
int stnry_cfg_set(stnry_sensitivity_t sensitivity);
 ...
 * @return Error code
 * @retval OK Success
 * @retval ERR_INTERNAL failed to read configuration
 * @retval ERR_INVAL_ADDR null pointer supplied
 */
int stnry_cfg_get(stnry_sensitivity_t* p_sensitivity);
 ...
 * @retval ERR_INVAL_ADDR null pointer supplied
 */
int uwb_cfg_set(uwb_cfg_t *p_cfg);
 ...
 * @return Error code
 * @retval OK Success
 * @retval ERR_INTERNAL Can't get configuration
 * @retval ERR_INVAL_ADDR null pointer supplied
 */
int uwb_cfg_get(uwb_cfg_t *p_cfg);
 ...
 * length is not enough(ERR_OVERRUN), this parameter contains expected lenght
 *
 * @return Error code
 * @retval > 0 Success
 * @retval 0 If no user data are available.
 * @retval ERR_INTERNAL Internal error
 * @retval ERR_OVERRUN If length provided is not enough.
 * @retval ERR_INVAL_ADDR Null pointer supplied
 */
int usr_data_read(uint8_t* p_data, uint8_t* p_len);
 ...
 * @return Error code
 * @retval len Success
 * @retval ERR_BUSY If overwrite == false and the device is busy
 * handling last request.
 * @retval ERR_INTERNAL No backhaul available
 * @retval ERR_INVAL_PARAM If length exceeds USR_DATA_LEN_MAX.
 * @retval ERR_INVAL_ADDR Null pointer supplied
 */
int usr_data_write(uint8_t* p_data, uint8_t len, bool overwrite);
 ...
 * length is not enough(ERR_OVERRUN) then this parameter contains expected
 * length LABEL_LEN_MAX
 *
 * @return Error code
 * @retval 0 Success
 * @retval ERR_INTERNAL Internal error
 ...
 * @retval ERR_INVAL_ADDR Null pointer supplied
 */
int label_read(uint8_t* p_label, uint8_t* p_len);
 ...
 * @retval len Success
 * @retval ERR_INTERNAL Internal error
 * @retval ERR_INVAL_PARAM If length exceeds LABEL_LEN_MAX.
 * @retval ERR_INVAL_ADDR Null pointer supplied
 */
int label_write(uint8_t* p_label, uint8_t len);
 ...
 */
typedef enum {
	GPIO_IDX_2 = 2,  //!< GPIO_IDX_2
	GPIO_IDX_8 = 8,  //!< GPIO_IDX_8
	GPIO_IDX_9 = 9,  //!< GPIO_IDX_9
	GPIO_IDX_10 = 10,  //!< GPIO_IDX_10
	GPIO_IDX_12 = 12,  //!< GPIO_IDX_12
	GPIO_IDX_13 = 13,  //!< GPIO_IDX_13
	GPIO_IDX_14 = 14,  //!< GPIO_IDX_14
	GPIO_IDX_15 = 15,  //!< GPIO_IDX_15
	GPIO_IDX_22 = 22,  //!< GPIO_IDX_22
	GPIO_IDX_23 = 23,  //!< GPIO_IDX_23
	GPIO_IDX_27 = 27,  //!< GPIO_IDX_27
	GPIO_IDX_30 = 30,  //!< GPIO_IDX_30
	GPIO_IDX_31 = 31 //!< GPIO_IDX_31
} gpio_idx_t;
...
int CheckGPIOIdx(gpio_idx_t idx);

/**
 * @brief Configures pin as output and sets the pin value
 *
 * @param[in] idx, Pin index (see gpio_idx_t)
 *
 * @param[in] value, Initial value of the output pin
 *
 * @return Error code
 */
int gpio_cfg_output(gpio_idx_t idx, bool value);
 ...
 * @param[in] idx, Pin index (see gpio_idx_t)
 *
 * @param[in] pull_mode, Pull mode of the input pin (see gpio_pin_pull_t)
 *
 * @return Error code
 */
int gpio_cfg_input(gpio_idx_t idx, gpio_pin_pull_t pull_mode);
 ...
 * @return Error code
 */
int gpio_value_set(gpio_idx_t idx, bool value);
 ...
 * @return Error code
 */
int gpio_value_get(gpio_idx_t idx, bool* value);
 ...
 * @return Error code
 */
int gpio_value_toggle(gpio_idx_t idx);
 ...
 * @return Error code
 * @retval OK Success
 * @retval ERR_INTERNAL Can't set PAN ID
 */
int panid_set(uint16_t panid);
 ...
 * @return Error code
 * @retval OK Success
 * @retval ERR_INTERNAL Can't read PAN ID
 * @retval ERR_INVAL_ADDR null pointer supplied
 */
int panid_get(uint16_t *p_panid);
 ...
 * @return Error code
 * @retval OK Success
 * @retval ERR_INTERNAL Can't read node ID
 * @retval ERR_INVAL_ADDR null pointer supplied
 */
int node_id_get(uint64_t *p_node_id);
 ...
 * @return Error code
 */
int status_get(status_t* p_status);
 ...
 * @return Error code
 */
int int_cfg_set(uint16_t value);
 ...
 * @return Error code
 */
int int_cfg_get(uint16_t *p_value);
...
typedef struct {
	...
	uint8_t origin_cnt;
	origin_info_t origin_info[API_BH_ORIGIN_CNT_MAX];
}bh_status_t;
/**
 ...
 */
int bh_status_get(bh_status_t * p_bh_status);
 ...
 * be disabled by clearing the key (enc_key_clear).
 ...
 * @return Error code
 * @retval OK Success
 * @retval ERR_INVAL_INTERNAL key can't be set or key is just all zeros
 * @retval ERR_INVAL_ADDR null pointer supplied
 */
int enc_key_set(enc_key_t* p_key);
 ...
 * @return Error code
 * @retval OK Success
 * @retval ERR_INVAL_INTERNAL can't be cleared
 */
int enc_key_clear(void);
...
typedef enum {
	uwb_PRAMBLE_CODE_9 = 9,
	uwb_PRAMBLE_CODE_10 = 10,
	uwb_PRAMBLE_CODE_11 = 11,
	uwb_PRAMBLE_CODE_12 = 12
} uwb_preamble_code_t;

/**
 * @brief Sets UWB preamble code
 *
 * @param[in] code One of the preamble codes in uwb_preamble_code_t
 *
 * @return Error code
 * @retval OK Success
 *
 * @retval ERR_INTERNAL DWM internal error
 * @retval ERR_INVAL_PARAM Invalid preamble code
 */
int uwb_preamble_code_set(uwb_preamble_code_t code); // todo
 ...
 * @retval ERR_INTERNAL DWM internal error
 */
int uwb_preamble_code_get(uwb_preamble_code_t *p_code); // todo


#define uwb_SCAN_RESULT_CNT_MAX 16
...
typedef struct {
	uint8_t cnt;
	uint8_t mode[uwb_SCAN_RESULT_CNT_MAX];
	int8_t rssi[uwb_SCAN_RESULT_CNT_MAX];
} uwb_scan_result_t;
 ...
 * @retval ERR_INVAL_ADDR null pointer supplied
 */
int uwb_scan_result_get(uwb_scan_result_t *p_result);
