...
static rs2::device get_device(size_t device_index = 0)
{
    // First, create a rs2::context.
    // The context represents the current platform with respect to connected devices
    rs2::context ctx;
...
static void list_sensors(const rs2::device& dev)
{
...
static rs2::sensor get_sensor(const rs2::device& dev, size_t device_index = 0)
{
    // A rs2::device is a container of rs2::sensors that have some correlation between them.
    // For example:
    //    * A device where all sensors are on a single board
...
static void print_intrinsics(const rs2_intrinsics &intrinsics)
{
...
static void print_intrinsics(const rs2_motion_device_intrinsic &intrinsics)
{
...
static void print_extrinsics(const rs2_extrinsics &extrinsics)
{
...
static float get_sensor_option(const rs2::sensor& sensor, rs2_option option_type)
{
    // Sensors usually have several options to control their properties
    //  such as Exposure, Brightness etc.
...
static void set_sensor_option(const rs2::sensor& sensor, rs2_option option_type, float set_value)
{
    // Sensors usually have several options to control their properties
    //  such as Exposure, Brightness etc.
    ...
    try
    {
        sensor.set_option(option_type, set_value);
    }
...
static void list_streaming_profiles(const std::vector<rs2::stream_profile>& stream_profiles)
{
...
static void list_streaming_profiles(const rs2::sensor& sensor)
{
    ...
    list_streaming_profiles(stream_profiles);
}

static rs2::stream_profile get_streaming_profile(const rs2::sensor& sensor, size_t selected_profile_index = 0)
{
    // Using the sensor we can get all of its streaming profiles
    std::vector<rs2::stream_profile> stream_profiles = sensor.get_stream_profiles();
...
static void test_streaming_profile(const rs2::sensor& sensor, const rs2::stream_profile& stream_profile)
{
